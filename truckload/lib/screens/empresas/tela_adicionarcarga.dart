...
class AdicionarCarga extends StatefulWidget {
  ...
  const AdicionarCarga({super.key, required this.empresaId});
  ...
  Future<void> _adicionarCarga() async {
    ...
    try {
      ...
      final dadosCarga = {
        ...
        'peso': peso,
        'preco': preco,
        'data': DateTime.now().add(const Duration(days: 7)).toIso8601String(),
        'status': 'disponivel',
        'regras': regrasController.text,
      };

      // Criar carga via API
      await _apiService.criarCargaEmpresa(dadosCarga);
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text("Erro ao adicionar carga: $e"),
            backgroundColor: Colors.red,
          ),
        );
      }
              ...
              children: [
                ...
                Center(
                  child: ElevatedButton(
                    onPressed: _isLoading ? null : _adicionarCarga,
        ...
        Container(
          ...
          child: Autocomplete<String>(
            optionsBuilder: (TextEditingValue value) {
              ...
              return todasCidades
                  .where(
                    (cidade) => cidade.toLowerCase().startsWith(
                      value.text.toLowerCase(),
                    ),
                  )
                  .take(10); // Limitar a 10 sugestões
            },
